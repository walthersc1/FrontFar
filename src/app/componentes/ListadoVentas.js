...
export default function ListadoVentas({nombre}) {
  const {isOpen, onOpen, onOpenChange} = useDisclosure();
  return (
    <>
      <button onClick={onOpen} className="flex bg-blue-900 gap-2 items-center text-white font-bold px-4 rounded-r-md"><Search className='w-5 h-5'></Search>Buscar {nombre}</button>
        {/* Encabezado */}
      ...
      <Modal isOpen={isOpen} size="5xl" onOpenChange={onOpenChange}>
        <ModalContent>
          {(onClose) => (
              <>
              <ModalHeader className="flex flex-col gap-1">Lista de {nombre}</ModalHeader>
              <ModalBody>
                  <div className='grid grid-cols-4 items-center'>
                    <div className='col-span-2'>
                      <input placeholder={`Digite el nombre del ${nombre}`} className="rounded-md bg-gray-50 p-2 border-2 w-full border-gray-700"></input>
                    </div>
                    ...
                    <div className='col-span-1 jestify-center'>
